...
def merge_api_vk_params(method, **add_params):
    default = dict(
        v='5.74',
        access_token=TOKEN
    )
    basic_url = 'https://api.vk.com/method/'
    url = '{}{}'.format(basic_url, method)
    return url, dict(default, **add_params)
    ...
    def __call__(self, *args, **kwargs):
        ...
        while True:
            try:
                ...
                log_error_request.debug('Something went wrong, '
                                        'trying for the {} time'.format(i))
                i += 1
                self.retries -= 1
...
def basic_request(method, **params):
    params = merge_api_vk_params(method, **params)
    response = requests.get(params[0], params[1])
    return response
...
def get_user_friends():
    ...
    method = 'friends.get'
    response = basic_request(method, source_uid=source_uid)
...
def get_user_brief_info(user_id):
    ...
    method = 'users.get'
    response = basic_request(method, user_ids=user_id)
...
def filter_user_live_friend_list():
    ...
    for user_id in user_friend_list:
        ...
        try:
            user_deactivated = (get_user_brief_info(user_id)
                                .json()['response'][0]['deactivated'])
...
def get_group_members(group_id):
    ...
    response = basic_request(method, group_id=group_id)
...
def is_user_in_group(friend, group):
    ...
    response = basic_request(method, user_ids=friend, group_id=group)
    return response
